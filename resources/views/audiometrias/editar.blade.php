<x-app-layout>
    ...
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">{{ __('Editar Audiometría') }}</h2>
    </x-slot>
    ...
    @if($errors->any())
        <div class="w-3/5 alert alert-danger rounded mt-4 mb-0 mx-auto" role="alert">
    ...
    <div class="py-12">
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                <form class="formEnviar" action="{{ route('audiometrias.update', $audiometria->id) }}" method="POST"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5 md:gap-8 my-4 mx-7">
                        <div class="grid grid-cols-1">
                            ...
                            <input name="rpe" id="rpe" value="{{$audiometria->rpe}}" readonly maxlength="5" type="text" required
                        ...
                        <div class="grid grid-cols-1">
                            ...
                            <select name="oido_izquierdo" id="oido_izquierdo" required
                                ...
                                @foreach ($resultados as $resultado)
                                    <option value="{{ $resultado->id }}" {{($audiometria->oido_izquierdo == $resultado->id) ? 'selected' : ''}}>{{ $resultado->diagnostico }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="grid grid-cols-1">
                            ...
                            <select name="oido_derecho" id="oido_derecho" required
                                ...
                                <option selected disabled hidden>Selecciona un resultado</option>
                                @foreach ($resultados as $resultado)
                                    <option value="{{ $resultado->id }}" {{($audiometria->oido_derecho == $resultado->id) ? 'selected' : ''}}>{{ $resultado->diagnostico }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="grid grid-cols-1">
                            <label for="fecha_toma"
                                ...
                                estudio:</label>
                            <input name="fecha_toma" id="fecha_toma" value="{{$audiometria->fecha_toma}}"
                        ...
                        <div class="grid grid-cols-1 position-relative">
                            ...
                            <input id="nuevoArchivo" name="archivo" onchange="validateFile(this)" type="file" accept=".pdf"
                                class="py-2 px-3 rounded-lg border-2 border-green-600 mt-1 focus:outline-none focus:ring-2 focus:ring-green-700 focus:border-transparent"
                                {{$errors->has('archivo') ? 'required' : 'hidden'}}
                                />
                            ...
                            @if(!$errors->has('archivo'))
                                <div id="archivoOriginal" class="py-2 px-3 rounded-lg border-2 border-green-600 mt-1 focus:outline-none focus:ring-2 focus:ring-green-700 focus:border-transparent d-flex flex-col">
                                    <a href="{{asset($archivo->filePath)}}" target="_blank">{{$archivo->nombre_archivo}}</a>
                    ...
                    <div class='flex items-center justify-center  md:gap-8 gap-4 pt-5 pb-5 mt-3'>
                        ...
                        <!-- botón enviar -->
                        @if(!session('message'))
                        <button type="submit" class="w-auto bg-green-500 hover:bg-green-700 rounded-lg shadow-xl font-medium text-black px-4 py-2">Guardar</button>
                        @endif
                    </div>
                </form>                  
            </div>
