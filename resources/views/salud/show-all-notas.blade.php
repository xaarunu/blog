        ...
        <div class="mx-auto sm:px-6 lg:px-8" style="width:80rem;">
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg px-6 " style="width:100%;">
                <table id="data-table" class="stripe hover translate-table"
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td align="center">{{ $sal->user->nombre.' '.$sal->user->paterno.' '.$sal->user->materno }}</td>
                            <td align="center">{{ $sal->fecha }}</td>
                            <td align="center">{{ $sal->user->getArea->area_nombre }}</td>
                            <td align="center">{{ $sal->user->getSubarea->subarea_nombre}}</td>
                            <td class="none">
                                <div class="flex justify-center rounded-lg text-lg" role="group" style="display:flex; gap:10px;">
                                    ...
                                    <form id="formEliminar-{{ $sal->id }}" action="{{ route('saluds.destroy', $sal->id) }}" method="POST"
                                        ...
                                        @csrf
                                        @method('DELETE')
                                        <button type="button" onclick="confirmDelete({{ $sal->id }});" class="rounded-lg text-black font-bold py-2 px-4 mx-auto">
                                            Borrar
                                        </button>
                                    </form>
                                </div>
                ...
                <div class="flex items-center justify-center  md:gap-8 gap-4 pb-5" style="display:flex; gap:10px;">
                    ...
                    <!-- botón crear -->
                    <a type="button" href="{{ route('saluds.create') }}" style="text-decoration:none"
...
<script>
    ...
    function filter() {
        ...
        fetch(SITEURL + '/saluds/filtro/show_allNotas',{
            method: 'POST',
            body: JSON.stringify({
                division: division,
                area: areaFilter,
            ...
            data.notas.forEach(nota => {
                ...
                var optionsBtns = `
                    ...
                    <!-- Botón Borrar -->
                    <form id="formEliminar-${nota.id}" action="` + urlUsar2 + `" method="POST"
                        class="rounded-lg bg-red-600 hover:bg-red-700 mx-auto">
                        @csrf
                        @method('DELETE')
                        <button type="button" onclick="confirmDelete(${nota.id})" class="rounded-lg text-black font-bold py-2 px-4 mx-auto">
                            Borrar
                        </button>
                    </form>
                    `;
