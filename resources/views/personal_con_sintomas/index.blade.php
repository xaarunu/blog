                ...
                <table id="data-table" class="stripe hover translate-table"
                    ...
                    <thead>
                        <tr>
                            ...
                            <th align="center">Fecha de detección</th>
                    ...
                    <tbody>
                        @foreach ($Personal_sintomas as $personal)
                        <tr>
                            <td align="center">{{ $personal->rpe }}</td>
                            <td align="center">{{ $personal->fecha_deteccion }}</td>
                            <td align="center">
                                @if(!$personal->nota->isEmpty())
                                        {{$personal->nota->last()->fecha}}
                                @else
                                        Sin nota médica.
                                @endif
                            </td>
                            {{-- <td align="center">{{ $personal->area }}</td> --}}
                            ...
                            <td class="none">
                                <div class="flex justify-center rounded-lg text-sm" role="group">
                                    <button class="mx-1 mt-3 border-right">
                                        <a href="{{ route('saluds.indice', $personal->rpe) }}" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded ">
                                            Seguimiento
                                        </a>
                                    </button>

                                    <form action="{{ route('personal_sintomas.marcarAtendido', $personal->id) }}" method="POST">
                                        @csrf
                                        @method('PUT')
...
<script>
    ...
    function filter() {
        ...
        fetch(SITEURL + '/filtroPersonalSintomas',{
            method: 'POST',
            body: JSON.stringify({
                division: division,
                area: areaFilter,
            ...
            data.users.forEach(users => {
                    var urlUsar = `{{route('saluds.indice','rpe')}}`;
                    urlUsar = urlUsar.replace('rpe',users.rpe);

                    var urlUsar2 = `{{route('personal_sintomas_atendido.show')}}`;
                    urlUsar2 = urlUsar2.replace('id',users.id);
                    ...
                    <button class="mx-1 mt-3 border-right">
                        <a href="`+ urlUsar2 + `" class="bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 rounded ">
                            Atendido
                        </a>
                    </button>
                    `;
                    var fecha = 0;
                    if (users.nota.length > 0) {
                        fecha = users.nota[0].fecha;
                    } else {
                        fecha = 'Sin nota médica.';
                    }

                dataTable.row.add([
                    users.rpe,
                    users.fecha_deteccion,
                    fecha,
                    users.user.get_area.area_nombre,
                    users.user.get_subarea.subarea_nombre,
                    optionsBtns
