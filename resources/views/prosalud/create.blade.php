<!-- resources/views/prosalud/create.blade.php{{-- El Santo, el Cavernario, Blue Demond y el Buldog --}} -->
<x-app-layout>
    @section('title', 'DCJ - CFE')
    <meta name="csrf-token" content="{{ csrf_token() }}">
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Crear Expediente ProSalud') }}
        </h2>
    </x-slot>
    <style>
        .alert {
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg pb-5">
                ...
                <form id="form_envio" action="{{ route('prosalud.store') }}" method="POST" class="formEnviar" enctype="multipart/form-data">
                    <div  id="datos_usuario">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-4 gap-5 md:gap-8 mt-5 mb-2 mx-7">
                            ...
                            <div class="grid grid-cols-1">
                                ...
                                <input id="fecha_toma" name="fecha_toma"
                                class="py-2 px-3 rounded-lg border-2 text-black border-green-500  mt-1 focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent"
                                    type="date" required />
                            </div>
                            <div class="grid grid-cols-1">
                            ...
                            <div class="grid grid-cols-1">
                                ...
                                <input id="glucosa_resultado" name="glucosa_resultado"
                                class="py-2 px-3 rounded-lg border-2 text-black  border-green-500  mt-1 focus:outline-none focus:ring-2 focus:ring-black focus:border-transparent "
                                    type="text" />
                            </div>
                            <div class="grid grid-cols-1">
                                <label class="block uppercase md:text-sm text-xs text-gray-500 font-semibold">Resultado de Trigliceridos:</label>
                                <input id="trigliceridos_resultado" name="trigliceridos_resultado"
                                class="py-2 px-3 rounded-lg border-2 text-black border-green-500  mt-1 focus:outline-none focus:ring-2 focus:ring-black focus:border-transparent "
                                    type="text" />
                            </div>
                            <div class="grid grid-cols-1">
                                <label class="block uppercase md:text-sm text-xs text-gray-500 font-semibold">Resultado del Colesterol:</label>
                                <input id="colesterol_resultado" name="colesterol_resultado"
                                class="py-2 px-3 rounded-lg border-2 text-black border-green-500  mt-1 focus:outline-none focus:ring-2 focus:ring-black focus:border-transparent  "
                                    type="text" />
                            </div>
                            <div class="grid grid-cols-1">
                                <label class="block uppercase md:text-sm text-xs text-gray-500 font-semibold">Resultado de la Hemoglobina:</label>
                                <input id="hemoglobina_resultado" name="hemoglobina_resultado"
                                class="py-2 px-3 rounded-lg border-2 text-black border-green-500  mt-1 focus:outline-none focus:ring-2 focus:ring-black focus:border-transparent "
                                    type="text" />
                            </div>
                            <div class="grid grid-cols-1">
                                <label class="block uppercase md:text-sm text-xs text-gray-500 font-semibold">Resultado de Leucocitos:</label>
                                <input id="leucocitos_resultado" name="leucocitos_resultado"
                                class="py-2 px-3 rounded-lg border-2 text-black border-green-500  mt-1 focus:outline-none focus:ring-2 focus:ring-black focus:border-transparent "
                                    type="text" />
                            </div>
                            <div class="grid grid-cols-1">
                                <label class="block uppercase md:text-sm text-xs text-gray-500 font-semibold">Resultado de Plaquetas:</label>
                                <input id="plaquetas_resultado" name="plaquetas_resultado"
                                class="py-2 px-3 rounded-lg border-2 text-black border-green-500  mt-1 focus:outline-none focus:ring-2 focus:ring-black focus:border-transparent "
                                    type="text" />
                            </div>
                            <div class="grid grid-cols-1">
                                <label class="block uppercase md:text-sm text-xs text-gray-500 font-semibold">Zona: <br></label>
                                <select id="zona" name="zona"
                                    class="block py-2 px-3 rounded-lg border-2 text-black border-green-500 mt-1 focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent">
                    ...
                    <div class='flex items-center justify-center  md:gap-8 gap-4 pt-5 pb-5'>
                        ...
                        <!-- botón enviar -->
                            <button type="submit" class="rounded-lg bg-green-500 hover:bg-green-700 font-medium text-black px-4 py-2">Crear</button>
                    </div>
                </form>
            </div>
        </div>
