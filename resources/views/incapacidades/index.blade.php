<x-app-layout>
    ...
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            @if(!isset($nombreCompleto) && (!isset($RijIncapacidad) || $RijIncapacidad == false))
            {{ __('Incapacidades') }}
            @else
    ...
    @if($incapacidades->isEmpty() )
        <div id="no-data-message" class="alert opacity-100 transition-opacity duration-500" style="color: #571515; background-color: #edd4d4; border: 1px solid #e6c3c3; position: relative; padding: 0.75rem 1.25rem; margin-bottom: 1rem; border-radius: 10px; margin: 10px;">
    ...
    <div class="py-10">
        ...
        @php $mostrarStatus = Auth::user()->hasAnyRole(['admin', 'Doctora']); @endphp
        <div class="grid grid-cols-{{ $mostrarStatus ? '5': '4' }} gap-5 md:gap-8 mx-7">
            ...
            <div class="grid grid-cols-1">
                <label class="block uppercase md:text-sm text-xs text-gray-500 font-semibold">Padecimiento:</label>
                <select id="padecimiento" name="padecimiento" class="py-2 px-3 rounded-lg border-2 border-green-600 mt-1 focus:outline-none focus:ring-2 focus:ring-green-700 focus:border-transparent" required>
            ...
            @can('incapacidad.autorizar')
                <div class="grid grid-cols-1">
                    ...
                    <select id="_estatus_filtro" name="estatus_filtro"
                        class="py-2 px-3 rounded-lg border-2 border-green-600 mt-1 focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent">
                        <option value="0" style='color: blue;'>Todos</option>
            ...
            <div class="flex justify-center gap-4 col-span-5">
                <div class="grid">
                    ...
                    <input  id="_fecha_inicio_filtro" name="fecha_inicio_filtro" type="date" {{--min="{{date('Y-m-d');}}"--}} class="py-2 px-3 rounded-lg border-2 border-green-500 focus:outline-none focus:ring-2 focus:ring-green-600 focus:border-transparent"  required/>
                </div>
                <div class="grid">
                    <label class="block uppercase md:text-sm text-xs text-gray-500 font-semibold">Fecha de fin:</label>
                    <input  id="_fecha_fin_filtro" name="fecha_fin_filtro" type="date" {{--min="{{date('Y-m-d');}}"--}} class="py-2 px-3 rounded-lg border-2 border-green-500 focus:outline-none focus:ring-2 focus:ring-green-600 focus:border-transparent"  required/>
                </div>
            </div>
        </div>
        <br>
            ...
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg px-6 border-4 p-4"
                ...
                @if (Auth::user()->hasAnyRole(['admin', 'Doctora']))
                    <div class="flex items-center justify-center  md:gap-8 gap-4 pb-5" style="display:flex; gap:10px;">
                        ...
                        <!-- botón crear -->
                        <a type="button" href="{{ route('incapacidades.create') }}" style="text-decoration:none"
